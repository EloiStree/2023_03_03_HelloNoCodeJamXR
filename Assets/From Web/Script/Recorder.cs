...
namespace Recorder
{
    ...
    public class Recorder : MonoBehaviour, IPointerClickHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        /// </summary>
        public PrimitiveUnityEvent_String ConsoleText;
        /// <summary>
        /// Show the Recording Time on the screen
        /// </summary>
        public PrimitiveUnityEvent_String RecordingTimeText;
      
        /// <summary>
        /// Record or Save Image for the Record Button
        /// </summary>
        public PrimitiveUnityEvent_Bool RecordImage, SaveImage;
        /// <summary>
        ...
        private void Update()
        {
            ...
            if (isRecording)
            {
                ...
                if (minute < 10)
                {
                    if (second < 10)
                    {
                        RecordingTimeText.Invoke("0" + minute + ":0" + second);
                    }
                    else
                    {
                        RecordingTimeText.Invoke(  "0" + minute + ":" + second);
                    }
                }
                else if (second < 10)
                {
                    RecordingTimeText.Invoke( minute + ":0" + second);
                }
                else
                {
                    RecordingTimeText.Invoke(  minute + ":" + second);
                }

                RecordImage.Invoke(false);
                SaveImage.Invoke(true);
            }
            else
            {
                RecordingTimeText.Invoke( "00:00");

                RecordImage.Invoke(true);
                SaveImage.Invoke(false);
            }
        }
        ...
        public void SaveRecording(string fileName = "Audio")
        {
            if (isRecording)
            {
                ...
                try
                {
                    ...
                    ConsoleText.Invoke( "Audio Saved at: " + filePath);
